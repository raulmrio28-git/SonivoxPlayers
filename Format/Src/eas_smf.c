...
#include "jet_data.h"
#endif
//Konakona False undefined identifier 20240904
#include <stdbool.h>
...
static EAS_RESULT SMF_ParseMetaEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream)
{
    ...
    else if (c == SMF_META_TEMPO)
    {
        ...
        // Konakona Universal platform Compile 20240904
#if __GNUC__ >= 5 || defined(__clang__)
        {
            // pSMFData->tickConv = (EAS_U16) (((temp * 1024) / pSMFData->ppqn + 500) / 1000);
            uint64_t temp64;
            if (mul_overflow(temp, 1024u, &temp64) ||
                    pSMFData->ppqn == 0 ||
                    (temp64 /= pSMFData->ppqn, false) ||
                    add_overflow(temp64, 500, &temp64) ||
                    (temp64 /= 1000, false) ||
                    temp64 > 65535) {
