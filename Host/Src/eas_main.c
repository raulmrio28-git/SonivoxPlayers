...
//Konakona EAS_OpenFile update patch 20240904
static EAS_FILE fLocator;
 ...
 *----------------------------------------------------------------------------
*/
static FILE* OpenFileHandle(const char* filename, const char* mode)
{
 ...
 *----------------------------------------------------------------------------
*/
int ReadFile(void* handle, void* buf, int offset, int size) {
...
static EAS_RESULT PlayFile (EAS_DATA_HANDLE easData, const char* filename, const char* outputFile, const S_EAS_LIB_CONFIG *pLibConfig, void *buffer, EAS_I32 bufferSize)
{
    ...
    wFile = NULL;
    fLocator.handle = OpenFileHandle(filename, "rb");
    fLocator.readAt = ReadFile;
    fLocator.size = SizeFile;
